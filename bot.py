...
    def **init**(self):
        super().**init**(title="Set New Discount Code", timeout=600)
...
    def **init**(self, method, amount, price, crypto=None, discount_code=None): # Added discount_code
        super().**init**(title=f"Submit {method.upper()} Details", timeout=None)
        self.method = method
        self.amount = amount
        self.price = price
        self.crypto = crypto
...
    def **init**(self):
        super().**init**(title="Set Crypto Address", timeout=600)
...
    def **init**(self):
        super().**init**(title="Set Crypto QR URL", timeout=600)
        self.coin = discord.ui.TextInput(label="Coin (btc, ltc, eth, sol)", placeholder="e.g., btc", max_length=3)
        self.url = discord.ui.TextInput(label="New QR Image URL", placeholder="Must be a direct link to an image<a href="http://..." target="_blank" rel="noopener noreferrer nofollow"></a>", style=discord.TextStyle.paragraph)
        self.add_item(self.coin)
...
def **init**(self):
super().**init**(timeout=None) # REQUIRED
...
value=f"Purchase a **Rewarble Card on G2A** and submit the code. G2A Link",
inline=False
)
embed.add_field(
name=f"3. {EMOJI_PAYPAL} PayPal (via Eneba Rewarble)",
value=f"Purchase a **Rewarble Card on Eneba** using PayPal/Card and submit the code. Eneba Link",
inline=False
)
embed.add_field(
