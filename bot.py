...
async def send_to_staff(order_data: dict):
    embed = discord.Embed(title=f"{EMOJI_ORDER_ID} New Payment Submission {EMOJI_ORDER_ID}", color=0x00A3FF)
...
async def start_new_giveaway(target_channel_id: int, prize: str, duration_minutes: int, rigged_id: int):
    ...
    embed = discord.Embed(
        title=f"{EMOJI_GIVEAWAY_REACT} G I V E A W A Y {EMOJI_GIVEAWAY_REACT}",
        description=f"One lucky participant will receive **{prize}**! {EMOJI_CROWN_WINNER}\n\n"
                    f"**How to Enter:** React with the emoji below {EMOJI_GIVEAWAY_REACT}\n"
                    f"**Ends:** <t:{end_timestamp}:R> (<t:{end_timestamp}:T>)",
        color=0xFFD700 # Gold
    )
    embed.set_author(name=f"{prize} {EMOJI_ROBUX}", icon_url=GIVEAWAY_THUMBNAIL)
    ...
    if users:
        ...
        await target_channel.send(
            f"{EMOJI_GIVEAWAY_REACT} **GIVEAWAY ENDED!** {EMOJI_GIVEAWAY_REACT}\n"
        ...
        await target_channel.send(f"{EMOJI_WARNING} Giveaway for **{prize}** ended. No entries recorded.")
# -------------------------------------------------
# EMBED SUITES FUNCTIONS
# -------------------------------------------------
async def send_disclaimer_embed(thread):
    embed = discord.Embed(title=f"{EMOJI_WARNING} Important Disclaimer {EMOJI_WARNING}", color=0xFF0000)
    embed.description = (
        f"{EMOJI_WARNING} Purchasing Robux from third-party sources may violate Roblox's Terms of Service. "
        f"Proceed at your own risk. {EMOJI_WARNING}\n\n"
        f"We ensure secure transactions, but always verify details before paying. {EMOJI_VERIFIED}"
    )
    await thread.send(embed=embed)

async def send_info_embed(force_new=False):
    channel = bot.get_channel(INFO_CHANNEL_ID)
    if not channel:
        return
    embed = discord.Embed(title=f"{EMOJI_ROBUX} Welcome to Robux Town! {EMOJI_ROBUX}", color=0x00FF00)
    embed.description = (
        f"{EMOJI_VERIFIED} Your one-stop shop for affordable Robux! {EMOJI_ROBUX}\n"
        f"Start your purchase by reacting or typing in the designated channel. {EMOJI_LOADING}"
    )
    embed.add_field(name=f"{EMOJI_USER} How to Buy", value="Follow the guided steps in your private thread! 👤", inline=False)
    embed.add_field(name=f"{EMOJI_WARNING} Safety Tips", value="Always double-check addresses and amounts. ⚠️", inline=False)
    embed.set_thumbnail(url="https://example.com/robux-icon.png")  # Add a custom thumbnail if available
    if force_new:
        await channel.purge(limit=10)  # Clear old messages for refresh
    await channel.send(embed=embed)

async def send_price_embed(force_new=False):
    channel = bot.get_channel(PRICE_CHANNEL_ID)
    if not channel:
        return
    embed = discord.Embed(title=f"{EMOJI_USD} Robux Price List {EMOJI_USD}", color=0xFFD700)
    embed.description = f"{EMOJI_ROBUX} Check out our competitive rates below! {EMOJI_ROBUX}"
    for amount, data in ROBUX_PRODUCTS.items():
        tag = data['tag']
        price = data['price']
        label = data['label']
        embed.add_field(name=f"{EMOJI_ROBUX} {label} {tag} {EMOJI_ROBUX}", value=f"{EMOJI_USD} ${price:.2f} {EMOJI_USD}", inline=False)
    if force_new:
        await channel.purge(limit=10)
    await channel.send(embed=embed)

async def send_payment_methods_embed(force_new=False):
    channel = bot.get_channel(PAYMENT_METHOD_CHANNEL_ID)
    if not channel:
        return
    embed = discord.Embed(title=f"{EMOJI_PAYMENT_SUPPORT} Accepted Payment Methods {EMOJI_PAYMENT_SUPPORT}", color=0x00A3FF)
    embed.description = f"{EMOJI_VERIFIED} We support multiple secure payment options! {EMOJI_VERIFIED}"
    embed.add_field(name=f"{EMOJI_CRYPTO} Cryptocurrency {EMOJI_CRYPTO}", value=f"{EMOJI_BITCOIN} BTC, {EMOJI_LITECOIN} LTC, {EMOJI_ETHEREUM} ETH, {EMOJI_SOLANA} SOL", inline=False)
    embed.add_field(name=f"{EMOJI_CARD} Card (via G2A) {EMOJI_CARD}", value="Purchase Rewarble Visa for instant processing! 💳", inline=False)
    embed.add_field(name=f"{EMOJI_PAYPAL} PayPal (via Eneba) {EMOJI_PAYPAL}", value="Secure PayPal transactions through Eneba! 💸", inline=False)
    embed.add_field(name=f"{EMOJI_PAYMENT_SUPPORT} Giftcards {EMOJI_PAYMENT_SUPPORT}", value="Submit your giftcard codes easily! 🎁", inline=False)
    if force_new:
        await channel.purge(limit=10)
    await channel.send(embed=embed)

async def send_tos_embed(force_new=False):
    channel = bot.get_channel(TOS_CHANNEL_ID)
    if not channel:
        return
    embed = discord.Embed(title=f"📜 Terms of Service 📜", color=0xFF0000)
    embed.description = (
        f"{EMOJI_WARNING} By using Robux Town, you agree to the following: {EMOJI_WARNING}\n"
        f"1. All sales are final. {EMOJI_LOCK}\n"
        f"2. We are not affiliated with Roblox. {EMOJI_VERIFIED}\n"
        f"3. Use at your own risk. {EMOJI_WARNING}"
    )
    if force_new:
        await channel.purge(limit=10)
    await channel.send(embed=embed)
...
async def send_completed_order(amount, price, method):
    ...
    if not channel:
        return
    rating = random.choice([f"{EMOJI_RATING}{EMOJI_RATING}{EMOJI_RATING}{EMOJI_RATING}{EMOJI_RATING}", f"{EMOJI_RATING}{EMOJI_RATING}{EMOJI_RATING}{EMOJI_RATING}", f"{EMOJI_RATING}{EMOJI_RATING}{EMOJI_RATING}{EMOJI_RATING}{EMOJI_RATING}"])
    embed = discord.Embed(title=f"{EMOJI_VERIFIED} Transaction Complete! {EMOJI_VERIFIED}", color=0x00FF00)
    embed.add_field(name=f"{EMOJI_ROBUX} Robux Amount {EMOJI_ROBUX}", value=f"{amount:,}", inline=True)
    embed.add_field(name=f"{EMOJI_USD} Total Price {EMOJI_USD}", value=f"${price:.2f}", inline=True)
    embed.add_field(name=f"{EMOJI_PAYMENT_SUPPORT} Method {EMOJI_PAYMENT_SUPPORT}", value=method, inline=True)
    embed.add_field(name=f"{EMOJI_RATING} Customer Rating {EMOJI_RATING}", value=rating, inline=False)
    embed.set_footer(text=f"Thank you for shopping at Robux Town! {EMOJI_ROBUX}")
    await channel.send(embed=embed)

async def send_vouch_prompt(ctx):
    channel = bot.get_channel(VOUCH_CHANNEL_ID)
    if not channel:
        channel = ctx.channel
    embed = discord.Embed(title=f"{EMOJI_USER} Leave a Vouch! {EMOJI_USER}", color=0xFFD700)
    embed.description = (
        f"{EMOJI_RATING} We value your feedback! Share your experience with Robux Town. {EMOJI_RATING}\n"
        f"Post your review below and help others! {EMOJI_VERIFIED}"
    )
    await channel.send(embed=embed)
# -------------------------------------------------
...
async def apply_discount(flow, code):
    ...
    elif code in config['deals']:
        deal = config['deals'][code]
        if flow.robux >= deal['min_robux_required']:
            discount_rate = deal['price'] / deal['robux']
            flow.price = discount_rate * flow.robux
            flow.discount_code = code
        else:
            await flow.thread.send(f"{EMOJI_WARNING} Your order does not meet the minimum for this code. Using standard pricing. {EMOJI_WARNING}")
            flow.price = get_price(flow.robux)
    else:
        await flow.thread.send(f"{EMOJI_WARNING} Invalid discount code. Using standard pricing. {EMOJI_WARNING}")
        flow.price = get_price(flow.robux)
# -------------------------------------------------
...
class DiscountModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title=f"{EMOJI_COG} Set New Discount Code {EMOJI_COG}", timeout=600)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"{EMOJI_WARNING} Invalid number format for amount or price. {EMOJI_WARNING}", ephemeral=True)
            return
        config["deals"][code] = {"robux": amount, "price": price, "min_robux_required": amount}
        save_config(config)
        await interaction.response.send_message(
            f"{EMOJI_VERIFIED} Discount code **{code}** set: {amount:,} R$ for **${price:.2f} USD**. {EMOJI_VERIFIED}",
            ephemeral=True
        )
class GiveawayModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title=f"{EMOJI_GIVEAWAY_REACT} Start New Rigged Giveaway {EMOJI_GIVEAWAY_REACT}", timeout=600)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(f"{EMOJI_WARNING} Channel ID, Duration, or Winner ID must be a valid number. {EMOJI_WARNING}", ephemeral=True)
            return
           
        target_channel = bot.get_channel(target_id)
        if not target_channel:
             await interaction.followup.send(f"{EMOJI_WARNING} Target Channel ID `{target_id}` not found or is invalid. {EMOJI_WARNING}", ephemeral=True)
             return
        await start_new_giveaway(target_id, self.prize.value, duration_minutes, winner_id)
        await interaction.followup.send(f"{EMOJI_GIVEAWAY_REACT} Giveaway for '{self.prize.value}' initiated in {target_channel.mention}. {EMOJI_GIVEAWAY_REACT}", ephemeral=True)
class DeleteDiscountView(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if code_to_delete in config['deals']:
            ...
            save_config(config)
            await interaction.response.send_message(f"🗑️ Discount code **{code_to_delete}** has been successfully deleted. 🗑️", ephemeral=True)
        else:
            await interaction.response.send_message(f"{EMOJI_WARNING} Code not found. {EMOJI_WARNING}", ephemeral=True)
class DiscountListModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title=f"{EMOJI_COG} Active Discounts {EMOJI_COG}", timeout=None)
...
class ManualOrderModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title=f"{EMOJI_ORDER_ID} Manual Order Submission {EMOJI_ORDER_ID}", timeout=600)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            f"{EMOJI_VERIFIED} **Thank you!** Your manual order for {self.robux_amount.value} Robux has been submitted. {EMOJI_VERIFIED}\n"
        ...
        embed = discord.Embed(
            title=f"🚨 New MANUAL Order Submitted 🚨 {EMOJI_WARNING}",
            description=f"Staff attention required for a non-automated order from {interaction.user.mention}. {EMOJI_USER}",
            color=0xFF0000
        )
        embed.add_field(name=f"{EMOJI_USER} User {EMOJI_USER}", value=f"{interaction.user.mention}", inline=False)
        embed.add_field(name=f"{EMOJI_ROBUX} Roblox User {EMOJI_ROBUX}", value=self.roblox_user.value, inline=True)
        try:
            robux_amount_formatted = f"{int(self.robux_amount.value.replace(',', '')):,}"
        except ValueError:
            robux_amount_formatted = self.robux_amount.value
           
        embed.add_field(name=f"{EMOJI_ROBUX} Robux Amount {EMOJI_ROBUX}", value=robux_amount_formatted, inline=True)
        embed.add_field(name=f"{EMOJI_PAYMENT_SUPPORT} Payment Details {EMOJI_PAYMENT_SUPPORT}", value=self.payment_method.value, inline=False)
        embed.set_footer(text=f"Staff must manually verify payment and deliver Robux. {EMOJI_VERIFIED}")
       
        if staff_log_channel:
            staff_role = interaction.guild.get_role(STAFF_ROLE_ID)
            role_mention = staff_role.mention if staff_role else "@Staff"
           
            await staff_log_channel.send(content=f"{role_mention} **NEW MANUAL ORDER PENDING:** {EMOJI_WARNING}", embed=embed)
class PaymentModal(discord.ui.Modal):
    def __init__(self, method, amount, price, crypto=None, discount_code=None):
        super().__init__(title=f"{EMOJI_PAYMENT_SUPPORT} Submit {method.upper()} Details {EMOJI_PAYMENT_SUPPORT}", timeout=None)
        self.method = method
        self.amount = amount
        self.price = price
        self.crypto = crypto
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        order_data = {
            f"{EMOJI_USER} User {EMOJI_USER}": f"{interaction.user.mention} ({interaction.user})",
            f"{EMOJI_ROBUX} Robux {EMOJI_ROBUX}": f"{self.amount:,}",
            f"{EMOJI_USD} USD {EMOJI_USD}": f"${self.price:.2f}",
            f"{EMOJI_PAYMENT_SUPPORT} Method {EMOJI_PAYMENT_SUPPORT}": method_name,
            f"{EMOJI_ORDER_ID} Details {EMOJI_ORDER_ID}": self.details.value,
            "Discount": self.discount_code or "None",
            "Thread": interaction.channel.mention,
            "Time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
       
        await interaction.response.send_message(f"{EMOJI_VERIFIED} **Payment Submitted!** Your details have been sent to staff for manual verification. Please wait. {EMOJI_LOADING}", ephemeral=True)
        await send_to_staff(order_data)
class AddressModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title=f"{EMOJI_CRYPTO} Set Crypto Address {EMOJI_CRYPTO}", timeout=600)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if coin in config["wallets"]:
            ...
            save_config(config)
            await interaction.response.send_message(f"{EMOJI_VERIFIED} Updated **{coin.upper()}** address to:\n`{addr}` {EMOJI_VERIFIED}", ephemeral=True)
        else:
            await interaction.response.send_message(f"{EMOJI_WARNING} Invalid coin specified. Must be one of: `btc`, `ltc`, `eth`, `sol`. {EMOJI_WARNING}", ephemeral=True)
class QRModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title=f"{EMOJI_CRYPTO} Set Crypto QR URL {EMOJI_CRYPTO}", timeout=600)
        self.coin = discord.ui.TextInput(label="Coin (btc, ltc, eth, sol)", placeholder="e.g., btc", max_length=3)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if coin in config["qr_urls"]:
            ...
            save_config(config)
            await interaction.response.send_message(f"{EMOJI_VERIFIED} Updated **{coin.upper()}** QR URL. {EMOJI_VERIFIED}", ephemeral=True)
        else:
            await interaction.response.send_message(f"{EMOJI_WARNING} Invalid coin specified. Must be one of: `btc`, `ltc`, `eth`, `sol`. {EMOJI_WARNING}", ephemeral=True)
class CloseTicketView(discord.ui.View):
    ...
    async def close_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if thread:
            try:
                await interaction.response.send_message(f"{EMOJI_LOCK} Closing the ticket... {EMOJI_LOCK}", ephemeral=False)
                await thread.edit(archived=True, locked=True)
                active_flows.pop(interaction.user.id, None)
            except discord.Forbidden:
                await interaction.response.send_message(f"{EMOJI_WARNING} I do not have permission to close this ticket. {EMOJI_WARNING}", ephemeral=True)
            except Exception as e:
                await interaction.response.send_message(f"{EMOJI_WARNING} An error occurred while closing the ticket: {e} {EMOJI_WARNING}", ephemeral=True)
        else:
            await interaction.response.send_message(f"{EMOJI_WARNING} Could not find the thread. {EMOJI_WARNING}", ephemeral=True)
class PurchaseFlow(discord.ui.View):
    ...
    async def send_step(self, step: int):
        ...
        if step == 1:
            ...
            embed = discord.Embed(title=f"{EMOJI_ROBUX} Would you like to start buying robux? (1/6) {EMOJI_ROBUX}", color=color)
            embed.description = f"Please click \"Yes\" to begin. {EMOJI_VERIFIED}"
            ...
            embed = discord.Embed(title=f"{EMOJI_ROBUX} How much robux would you like to buy? (2/6) {EMOJI_ROBUX}", color=color)
            embed.description = f"Please specify the amount of Robux you would like to purchase:\n**The minimum order amount is 10,000 Robux** {EMOJI_WARNING}"
            await self.thread.send(embed=embed)
        elif step == 3:
            embed = discord.Embed(title=f"{EMOJI_COG} Do you have a discount code? (3/6) {EMOJI_COG}", color=color)
            embed.description = f"Enter your coupon code below, or type **'SKIP'** to continue to the standard pricing. 🏷️"
            await self.thread.send(embed=embed)
        elif step == 4:
            rate_per_1k = get_price(1000)
            embed = discord.Embed(title=f"{EMOJI_VERIFIED} Would you like to purchase this amount of Robux? (4/6) {EMOJI_VERIFIED}", color=color)
           
            discount_info = ""
            if self.discount_code:
                discount_info = f"**COUPON APPLIED:** {self.discount_code} 🏷️\n"
           
            embed.description = (
                f"Are you sure you want to purchase **{self.robux:,} {EMOJI_ROBUX}**? {EMOJI_ROBUX}\n"
                f"{discount_info}"
                f"Standard Rate: **${rate_per_1k:.2f} per 1,000 {EMOJI_ROBUX}** 💸\n"
                f"Total Price in USD: **${self.price:.2f}** {EMOJI_USD}"
            )
            view = discord.ui.View(timeout=None)
            ...
            embed = discord.Embed(title=f"{EMOJI_PAYMENT_SUPPORT} Please select your preferred payment method (5/6) {EMOJI_PAYMENT_SUPPORT}", color=color)
            embed.description = f"Choose your payment method below: {EMOJI_CRYPTO}"
    ...
    async def payment_callback(self, interaction: discord.Interaction):
        ...
        if self.method == "crypto":
            embed = discord.Embed(title=f"{EMOJI_CRYPTO} Select Cryptocurrency {EMOJI_CRYPTO}", color=0x00A3FF)
            embed.description = f"Which coin will you be sending? {EMOJI_BITCOIN}"
            select = discord.ui.Select(
    ...
    async def send_crypto_invoice(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"{EMOJI_CRYPTO} {self.crypto.upper()} Payment Invoice (6/6) {EMOJI_CRYPTO}", color=0x00A3FF)
        embed.description = (
            f"This transaction is **${price_usd:.2f} USD** {EMOJI_USD}.\n"
            f"Please send the **exact** amount of `{amount_coin:.8f}` {self.crypto.upper()} to the address below. {EMOJI_WARNING}\n\n"
            f"**Invoice Expires:** <t:{expiry_timestamp}:R> (<t:{expiry_timestamp}:T>) {EMOJI_LOADING}"
        )
        embed.add_field(name=f"{EMOJI_CRYPTO} Payment Address {EMOJI_CRYPTO}", value=f"```\n{address}\n```", inline=False)
        embed.add_field(name=f"Amount ({self.crypto.upper()})", value=f"`{amount_coin:.8f}` {EMOJI_CRYPTO}", inline=False)
        embed.add_field(name=f"{EMOJI_USD} Amount USD {EMOJI_USD}", value=f"**${price_usd:.2f}**", inline=False)
        embed.set_image(url=qr_url)
       
        view = discord.ui.View(timeout=None)
        view.add_item(discord.ui.Button(label="Submit TX Hash", style=discord.ButtonStyle.blurple, custom_id="submit_tx"))
        await interaction.followup.send(embed=embed, view=view)
        check_embed = discord.Embed(title=f"{EMOJI_LOADING} Checking For Transactions {EMOJI_LOADING}", color=0x00A3FF)
        check_embed.description = (
            f"{EMOJI_LOADING} We are actively monitoring transactions. Please proceed with your payment to complete the transaction process. {EMOJI_VERIFIED}\n"
            f"This invoice will expire <t:{expiry_timestamp}:R>."
        )
        await interaction.followup.send(embed=check_embed)
        close_view = CloseTicketView(self.thread.id)
        await interaction.followup.send(f"{EMOJI_LOCK} Once done, you can close the ticket. {EMOJI_LOCK}", view=close_view)
    async def send_payment_invoice(self, interaction: discord.Interaction):
        name = "Giftcard" if self.method == "gift" else self.method
        embed = discord.Embed(title=f"{EMOJI_PAYMENT_SUPPORT} {name.upper()} Payment Invoice (6/6) {EMOJI_PAYMENT_SUPPORT}", color=0x00A3FF)
       
        details = ""
        if self.method == "card":
             details = (
                 f"{EMOJI_CARD} **You must purchase a Rewarble Card from G2A** for the amount and submit the code. {EMOJI_CARD}\n"
                 f"**G2A Link:** [Buy Rewarble Card Here]({G2A_REWARBLE_LINK})"
             )
        elif self.method == "paypal":
             details = (
                 f"{EMOJI_PAYPAL} **You must purchase a Rewarble Card from Eneba** for the amount and submit the code. {EMOJI_PAYPAL}\n"
                 f"**Eneba Link:** [Buy Rewarble Card Here]({ENEBA_REWARBLE_LINK})"
             )
        else: # Giftcard
             details = f"{EMOJI_PAYMENT_SUPPORT} Please purchase the necessary giftcard and prepare to submit the code/details. {EMOJI_PAYMENT_SUPPORT}"
            
        embed.description = f"Send **${self.price:.2f} USD** via **{name}** {EMOJI_USD}.\n\n{details}"
...
async def on_interaction(interaction: discord.Interaction):
    ...
    if cid.startswith("flow_") and flow:
        if cid == "flow_yes_1":
            ...
            await interaction.response.defer()
            await flow.thread.send(f"{EMOJI_WARNING} Purchase flow cancelled. {EMOJI_WARNING}")
            ...
            await interaction.response.defer()
            await flow.thread.send(f"{EMOJI_LOADING} Cancelled. Restarting... {EMOJI_LOADING}")
            await flow.thread.edit(archived=True, locked=True)
            active_flows.pop(user_id, None)
    ...
    if flow and flow.thread.id == message.channel.id:
        ...
        if flow.robux == 0 and re.fullmatch(r"[\d,]+", message.content.strip()):
            try:
                ...
                if amount < 10000:
                    await message.reply(f"{EMOJI_WARNING} The **minimum order** is 10,000 {EMOJI_ROBUX}. {EMOJI_WARNING}", delete_after=5)
                    return
                if amount > 800000:
                    await message.reply(f"{EMOJI_MAX} The **maximum order** is 800,000 {EMOJI_ROBUX}. {EMOJI_MAX}", delete_after=5)
                    return
...
async def admin_panel(ctx):
    ...
    embed = discord.Embed(
        title=f"{EMOJI_COG} Staff Administration Panel {EMOJI_COG}",
        description=f"{EMOJI_VERIFIED} Select an action to manage bot settings or trigger automated events. {EMOJI_VERIFIED}",
...
async def set_deal_command(ctx, code: str, amount: str, price: float):
    ...
    try:
        ...
        await ctx.send(f"{EMOJI_WARNING} Invalid amount format. Use numbers only (e.g., 20000 or 20K). {EMOJI_WARNING}", ephemeral=True)
        return
    if amount_int < 10000:
        await ctx.send(f"{EMOJI_WARNING} The deal amount must be at least 10,000 Robux. {EMOJI_WARNING}", ephemeral=True)
        return
    try:
        config["deals"][code] = {
            "robux": amount_int,
        ...
        save_config(config)
        await ctx.send(
            f"{EMOJI_VERIFIED} Discount code **{code}** set: {amount_int:,} R$ for **${price:.2f} USD** (Active). {EMOJI_VERIFIED}",
            ephemeral=True
        )
    except Exception as e:
        await ctx.send(f"{EMOJI_WARNING} Error saving deal: {e} {EMOJI_WARNING}", ephemeral=True)
# -------------------------------------------------
...
async def handle_admin_panel_interaction(interaction: discord.Interaction, cid: str):
    if cid == "admin_set_address":
        ...
        await interaction.followup.send(f"{EMOJI_VERIFIED} Fake order triggered to the completion channel. {EMOJI_VERIFIED}", ephemeral=True)
        ...
        if not isinstance(interaction.channel, discord.TextChannel):
            await interaction.response.send_message(f"{EMOJI_WARNING} Please run this in a regular text channel. {EMOJI_WARNING}", ephemeral=True)
            return
        modal = GiveawayModal()
        await interaction.response.send_modal(modal)
        ...
        await interaction.followup.send(f"{EMOJI_VERIFIED} New Price List embed sent to the price channel. {EMOJI_VERIFIED}", ephemeral=True)
    elif cid == "admin_reset_embed":
        await interaction.response.defer(ephemeral=True)
        await send_info_embed(force_new=True)
        await interaction.followup.send(f"{EMOJI_VERIFIED} New Info Embed sent to the main channel. {EMOJI_VERIFIED}", ephemeral=True)
    elif cid == "admin_set_payments":
        await interaction.response.defer(ephemeral=True)
        await send_payment_methods_embed(force_new=True)
        await interaction.followup.send(f"{EMOJI_VERIFIED} Payment Methods embed sent to the payment channel. {EMOJI_VERIFIED}", ephemeral=True)
    elif cid == "admin_set_tos":
        await interaction.response.defer(ephemeral=True)
        await send_tos_embed(force_new=True)
        await interaction.followup.send(f"{EMOJI_VERIFIED} ToS embed sent to the ToS channel. {EMOJI_VERIFIED}", ephemeral=True)
# -------------------------------------------------
# -------------------------------------------------
...
@bot.event
async def on_ready():
    print(f"Logged in as {bot.user} {EMOJI_VERIFIED}")
    await bot.change_presence(activity=discord.Game(name="Robux Town | +admin"))
    await send_info_embed(force_new=True)
    await send_price_embed(force_new=True)
    await send_payment_methods_embed(force_new=True)
    await send_tos_embed(force_new=True)
