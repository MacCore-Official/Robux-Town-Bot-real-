...
async def send_price_embed(force_new: bool = False):
    ...
    # Define embed BEFORE checking history to avoid NameError
    embed = discord.Embed(
    ...
    # Check history AFTER defining embed
    try:
        if not force_new:
...
async def send_info_embed(force_new: bool = False):
    ...
    embed.description = (
        ...
        "**Payment Options**\n"
        f"• 🪙 Crypto (BTC/LTC/ETH/SOL)\n" # Using Unicode fallback for simplicity in this embed
        f"• 💳 Card (G2A)\n"
        f"• 💵 PayPal (Eneba)\n"
        f"• 🎁 Giftcards\n\n"
...
async def on_ready():
    ...
    # All necessary embed functions are now defined ABOVE this point.
