...
active_flows = {}  # Added: Missing global dictionary for tracking user flows
...
class PurchaseFlow(discord.ui.View):
    ...
    async def send_payment_invoice(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(embed=embed, view=view)
# -------------------------------------------------
# Added: Missing apply_discount function
# -------------------------------------------------
async def apply_discount(flow, code):
    ...
    if code == 'SKIP':
        ...
        if deal and flow.robux >= deal['min_robux_required']:
            ...
            flow.price = get_price(flow.robux)
            flow.discount_code = None
            await flow.thread.send(f"{EMOJI_WARNING} Discount code invalid or not applicable. Using standard price.")
# -------------------------------------------------
# Added: Missing send_* functions with placeholders (customize as needed)
# -------------------------------------------------
async def send_disclaimer_embed(channel):
    embed = discord.Embed(
        title="Important Disclaimer",
        description="By using Robux Town, you agree to our terms. Purchases are final, no refunds. We are not affiliated with Roblox Corporation. Ensure your account is secure.",
        color=0xFFFF00
    )
    await channel.send(embed=embed)
...
async def send_completed_order(amount, price, method):
    ...
    if channel:
        ...
        embed.add_field(name="Robux Amount", value=f"{amount:,} {EMOJI_ROBUX}", inline=True)
...
async def send_vouch_prompt(ctx):
    ...
    if channel:
        embed = discord.Embed(title="Leave a Vouch! ⭐", description="Share your experience with Robux Town. How was your purchase?", color=0xFFD700)
        await channel.send(embed=embed)
    await ctx.send(f"{EMOJI_VERIFIED} Vouch prompt sent to the vouch channel.", ephemeral=True)
        ...
        if force_new:
            async for msg in channel.history(limit=100):
                if msg.author == bot.user:
                    await msg.delete()
        embed = discord.Embed(title=f"{EMOJI_ROBUX} Robux Price List {EMOJI_ROBUX}", color=0x00A3FF)
        for amount, data in ROBUX_PRODUCTS.items():
            embed.add_field(name=f"{data['label']} {data['tag']}", value=f"${data['price']:.2f}", inline=False)
        await channel.send(embed=embed)
    ...
    if channel:
        if force_new:
            async for msg in channel.history(limit=100):
                if msg.author == bot.user:
                    await msg.delete()
        embed = discord.Embed(title="Welcome to Robux Town™", description="Buy cheap, safe Robux here! Click below to start your purchase.", color=0x00A3FF)
        view = discord.ui.View(timeout=None)
        view.add_item(discord.ui.Button(label="Start Purchase", style=discord.ButtonStyle.green, custom_id="start_purchase"))
    ...
    if channel:
        if force_new:
            async for msg in channel.history(limit=100):
                if msg.author == bot.user:
                    await msg.delete()
        embed = discord.Embed(title="Accepted Payment Methods", color=0x00A3FF)
        embed.add_field(name="Cryptocurrency", value="BTC, LTC, ETH, SOL", inline=False)
        embed.add_field(name="Card", value="Via G2A Rewarble", inline=False)
        embed.add_field(name="PayPal", value="Via Eneba Rewarble", inline=False)
        embed.add_field(name="Giftcards", value="Various supported", inline=False)
        await channel.send(embed=embed)
    ...
    if channel:
        if force_new:
            async for msg in channel.history(limit=100):
                if msg.author == bot.user:
                    await msg.delete()
        embed = discord.Embed(title="Terms of Service", color=0xFF0000)
        embed.description = "1. All sales are final - no refunds.\n2. You are responsible for your Roblox account security.\n3. We reserve the right to refuse service.\n4. By using this bot, you agree to these terms."
        await channel.send(embed=embed)
# -------------------------------------------------
...
async def on_interaction(interaction: discord.Interaction):
    ...
    if cid == "start_purchase":  # Added: Handler to start purchase flow from info embed button
        await interaction.response.defer()
...
# Added: Basic on_ready event to confirm bot startup
@bot.event
async def on_ready():
